...
const migration = readFileSync(path.join(root, "supabase", "migrations", "20260921203000_secure_permanent_tournament_deletion.sql"), "utf8");
...
test("permanent delete migration defines secure security-definer RPC with authorization and audit logging", () => {
...
test("permanent delete migration enforces strict foreign-key cascade order for all tournament-owned tables", () => {
  ...
  assert.match(migration, /delete from public\.ball_by_ball/);
...
test("permanent delete migration preserves reusable teams and global players", () => {
...
test("prevent_tournament_hard_delete trigger blocks direct client delete while allowing authorized RPC", () => {
  assert.match(migration, /create or replace function public\.prevent_tournament_hard_delete\(\)/);
  assert.match(migration, /current_setting\('crickpulse\.allow_tournament_delete', true\) = 'on'/);
  assert.match(migration, /Direct tournament deletion is disabled/);
  assert.match(migration, /perform set_config\('crickpulse\.allow_tournament_delete', 'on', true\);/);
  assert.match(migration, /delete from public\.tournaments where id = p_tournament_id;/);
  assert.match(migration, /revoke delete on public\.tournaments from authenticated, anon, public;/);
});

test("RPC execution privileges are restricted to authenticated users only", () => {
