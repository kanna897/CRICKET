...
export function useLiveScoringPage() {
    ...
    <section className="mobile-portrait-scorer" aria-label="Portrait quick scoring">
      ...
      <div className="mobile-score-strip"><div><small>{innings ? `${teamName(innings.batting_team_id)} batting` : "Innings"}</small><strong>{score}</strong></div><div><small>Overs</small><strong>{overs}/{effectiveOvers}</strong></div><div><small>{innings?.innings_number === 2 ? "Target" : "CRR"}</small><strong>{innings?.innings_number === 2 ? innings.target || "—" : currentRunRate.toFixed(2)}</strong></div></div>
      {freeHitActive ? <p className="mobile-free-hit">Free Hit</p> : null}
      <div className="mobile-player-strip"><div><small>Striker</small><strong>{playerName(innings?.striker_id || null)}</strong><span>{strikerStats.runs} ({strikerStats.balls}) *</span></div><div><small>Non-striker</small><strong>{playerName(innings?.non_striker_id || null)}</strong><span>{nonStrikerStats.runs} ({nonStrikerStats.balls})</span></div><div><small>Bowler</small><strong>{playerName(innings?.current_bowler_id || null)}</strong><button type="button" onClick={() => { setNewBowler(""); setNextBowlerOpen(true); }} disabled={!innings || saving || completed}>Change</button></div></div>
      ...
      <div className="mobile-utility-grid"><button type="button" onClick={() => openAdvancedDelivery()} disabled={saving || completed || requiresSetup}><SlidersHorizontal />More Extras</button><button type="button" onClick={undoLastBall} disabled={!balls.length || saving || completed}><RotateCcw />Undo Last Ball</button></div>
