...
import { deliveryBadgeClass, deliveryBadgeLabel } from "@/features/scoring/utils";
...
export function useLiveScoringPage() {
    ...
    const [isMobileWicketSelector, setIsMobileWicketSelector] = useReducerState(false);
    ...
    const record = async ({ runs = 0, extras = 0, extrasType, legal = true, wicket = false, dismissalType, outId, fielderId, shotZone }: {
        ...
        if (wicket && dismissalType)
            setWicketPop({ type: dismissalType, key: crypto.randomUUID() });
        setSaving(true);
        try {
            ...
            const matchResult = innings.innings_number === 2 && inningsComplete
                ? targetReached
                    ? `MATCH WON! ${teamName(innings.batting_team_id)} win by ${Math.max((match.wickets_per_innings || 10) - nextTotalWickets, 0)} wicket${Math.max((match.wickets_per_innings || 10) - nextTotalWickets, 0) === 1 ? "" : "s"}.`
            ...
            // A completed innings opens its own setup screen. Do not ask for a
            // bowler from the previous innings at the same time.
            if (isOverEnd && !inningsComplete) {
                setNewBowler("");
    ...
    const swapStrike = async () => {
        if (!innings || saving || completed || !innings.striker_id || !innings.non_striker_id)
            return;
        setSaving(true);
        try {
            await saveInnings({
            ...
            alert("Unable to swap strike.");
        }
        finally {
            setSaving(false);
        }
    };
    const openWicket = (type: string = "bowled", isMobile: boolean = false) => {
        setIsMobileWicketSelector(isMobile);
        setWicketType(type);
    ...
    const saveWicket = async () => {
        ...
        if (wicketType === "retired_hurt") {
            ...
            setSaving(true);
            try {
                const isStriker = playerOut === innings.striker_id;
    ...
    const startVoiceScoring = () => {
        ...
        recognition.onresult = (event) => {
            ...
            if (command.includes("no ball") && match?.allow_no_balls)
                ...
                openWicket(freeHitActive ? "run_out" : "bowled", true);
            else if (runCommand)
      ...
      <div className="mobile-over-ribbon">
        <div className="mobile-over-balls-track">
          {currentOverBalls.length ? currentOverBalls.map((ball) => {
            const label = deliveryBadgeLabel(ball);
            const isSix = ball.runs === 6;
            const isFour = ball.runs === 4;
            const isDot = !ball.is_wicket && !ball.extras_type && ball.runs === 0;
            return (
              <span
                key={ball.id}
                title={`Over ${ball.over_number}.${ball.ball_number} - ${label}`}
                className={`mobile-over-ball ${ball.is_wicket ? "ball-wicket" : ball.extras_type ? "ball-extra" : isSix ? "ball-six" : isFour ? "ball-boundary" : isDot ? "ball-dot" : "ball-runs"}`}
              >
                {label}
              </span>
            );
          }) : <em className="text-[0.68rem] font-semibold text-sky-200/70">No deliveries in this over yet</em>}
        </div>
      </div>
    ...
    {wicketOpen && <Modal title={isMobileWicketSelector ? `Record Wicket — ${wicketType.replaceAll("_", " ")}` : `Wicket — ${wicketType.replaceAll("_", " ")}`} onClose={() => setWicketOpen(false)}>
      <div className="space-y-3">
        {isMobileWicketSelector && (
          <div>
            <label className="mb-1.5 block text-xs font-black uppercase tracking-wider text-muted-foreground">Dismissal Type</label>
            {freeHitActive ? (
              <p className="rounded-lg bg-amber-100 px-3 py-2 text-xs font-bold text-amber-950">Free Hit active — only Run Out is available.</p>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      setWicketType(type);
                      if (!["caught", "run_out", "stumped"].includes(type)) setFielder("");
                    }}
                    className={`rounded-lg border px-2 py-2 text-xs font-black transition ${
                      wicketType === type
                        ? "border-red-500 bg-red-500 text-white shadow-sm"
                        : "border-border bg-card text-foreground hover:border-red-400"
                    }`}
                  >
                    {label}
                  </button>
    ...
    {advancedDeliveryOpen && <Modal title="Advanced delivery" onClose={() => setAdvancedDeliveryOpen(false)}>
      <Select label="Extras type" value={advancedExtrasType} onChange={(value) => { setAdvancedExtrasType(value); setAdvancedRuns(0); setAdvancedExtras(value === "wide" || value === "no_ball" ? 1 : 0); }} options={([...(match.allow_wides ? [["wide", "Wide"]] : []), ...(match.allow_no_balls ? [["no_ball", "No ball"]] : []), ["bye", "Bye"], ["leg_bye", "Leg bye"]] as string[][])}/>
      <NumberChoice label="Runs off the bat (no-ball only)" value={advancedRuns} values={[0, 1, 2, 3, 4, 5, 6]} onChange={setAdvancedRuns} disabled={advancedExtrasType !== "no_ball"}/>
      <NumberChoice label={advancedExtrasType === "wide" || advancedExtrasType === "no_ball" ? "Additional extra runs (mandatory 1 is added automatically)" : "Bye / leg-bye runs"} value={advancedExtrasType === "wide" || advancedExtrasType === "no_ball" ? advancedExtras - 1 : advancedExtras} values={advancedExtrasType === "wide" || advancedExtrasType === "no_ball" ? [0, 1, 2, 3, 4, 5, 6] : [1, 2, 3, 4, 5, 6]} onChange={(value) => setAdvancedExtras(advancedExtrasType === "wide" || advancedExtrasType === "no_ball" ? value + 1 : value)}/>
      <p className="rounded-lg bg-muted p-3 text-xs text-muted-foreground">This records one <strong className="text-foreground">{advancedExtrasType === "no_ball" ? advancedRuns + advancedExtras > 1 ? `NB+${advancedRuns + advancedExtras - 1}` : "NB" : advancedExtrasType === "wide" ? advancedExtras > 1 ? `Wd+${advancedExtras - 1}` : "Wd" : `${advancedExtrasType === "bye" ? "B" : "LB"}${advancedExtras}`}</strong> delivery. {(advancedExtrasType === "wide" || advancedExtrasType === "no_ball") && <>The mandatory {advancedExtrasType === "wide" ? "wide" : "no-ball"} is included automatically. </>}Total added: <strong className="text-foreground">{advancedRuns + advancedExtras}</strong>.</p>
      <ModalActions onCancel={() => setAdvancedDeliveryOpen(false)} onSave={saveAdvancedDelivery} saving={saving} label="Record delivery"/>
    </Modal>}
