...
export function TournamentEditor({ tournament, snapshot }: { tournament: EditableTournament; snapshot: TournamentSnapshot }) {
  ...
  function chooseImage(file: File | undefined, kind: "logo" | "banner") {
    if (!file) return;
    ...
    if (kind === "logo") { if (logoPreview) URL.revokeObjectURL(logoPreview); setLogoFile(file); setLogoPreview(preview); }
    else { if (bannerPreview) URL.revokeObjectURL(bannerPreview); setBannerFile(file); setBannerPreview(preview); }
  }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      let bannerUrl = tournament.banner_url;
      if (logoFile) ({ url: logoUrl } = await uploadImage(logoFile, "tournament-logos"));
      if (bannerFile) ({ url: bannerUrl } = await uploadImage(bannerFile, "banners"));
      const { data, error } = await supabase.from("tournaments").update({ ...form, tournament_name: form.name, overs: Number(form.overs), overs_per_match: Number(form.overs), logo_url: logoUrl, banner_url: bannerUrl }).eq("id", tournament.id).select("id").maybeSingle();
      if (error) throw error;
      if (!data) throw new Error("Unauthorized: you cannot update this tournament.");
      setMessage("Tournament details, logo and banner saved."); setLogoFile(null); setBannerFile(null); router.refresh();
  ...
  return <div className="admin-themed-page mx-auto max-w-5xl space-y-6">
    ...
    <form onSubmit={save} className="space-y-5 rounded-xl border border-border bg-card p-6 text-foreground shadow-sm">
      ...
      <div className="grid gap-5 lg:grid-cols-[13rem_1fr]"><MediaPicker label="Tournament logo" help="Square image recommended" preview={logoPreview || tournament.logo_url} aspect="square" onFile={(file) => chooseImage(file, "logo")}/><MediaPicker label="Tournament banner" help="Wide 16:6 image recommended · shown behind the logo on landing page" preview={bannerPreview || tournament.banner_url} aspect="banner" onFile={(file) => chooseImage(file, "banner")}/></div>
...
function MediaPicker({ label, help, preview, aspect, onFile }: { label: string; help: string; preview: string | null; aspect: "square" | "banner"; onFile: (file?: File) => void }) { return <div><p className="mb-2 text-sm font-black">{label}</p><div className={`relative overflow-hidden rounded-2xl border border-border bg-background ${aspect === "square" ? "aspect-square" : "aspect-[16/6]"}`}>{preview ? <Image unoptimized fill sizes={aspect === "square" ? "13rem" : "700px"} src={preview} alt={`${label} preview`} className={aspect === "square" ? "object-contain p-3" : "object-cover"}/> : <span className="grid h-full place-items-center"><ImagePlus className="h-10 w-10 text-muted-foreground"/></span>}</div><label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-xl border border-primary px-4 py-2 text-sm font-black text-primary"><ImagePlus className="h-4 w-4"/>Upload / Change<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => { onFile(event.target.files?.[0]); event.target.value = ""; }}/></label><p className="mt-2 text-xs text-muted-foreground">{help} · JPG, PNG or WebP · max 5 MB</p></div>; }
