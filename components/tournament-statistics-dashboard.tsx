...
export function TournamentStatisticsDashboard({ admin = false, organizerId, isMasterAdmin = false }: { admin?: boolean; organizerId?: string; isMasterAdmin?: boolean }) {
  ...
  const downloadPng = async () => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.click();
    } catch { setMessage("Report image could not be generated. Please try again."); }
  ...
  const shareReport = async () => {
    ...
    else { await navigator.clipboard.writeText(window.location.href); setMessage("Statistics link copied to clipboard."); }
  };

  return <div className={`mx-auto max-w-6xl space-y-6 ${text}`}><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.2em] text-primary">Performance intelligence</p><h1 className="mt-1 text-3xl font-black">Tournament Statistics</h1><p className={`mt-2 ${admin ? "text-muted-foreground" : "text-slate-500"}`}>Automatic batting and bowling leaders from verified ball-by-ball records.</p></div><div className="flex gap-2"><select aria-label="Tournament" value={selectedTournament} onChange={(event) => setSelectedTournament(event.target.value)} className={`min-w-0 flex-1 rounded-xl border px-3 py-2.5 font-bold sm:min-w-64 ${card}`}>{tournaments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button type="button" onClick={() => void load()} disabled={loading || !selectedTournament} aria-label="Refresh statistics" className={`rounded-xl border px-3 text-primary disabled:opacity-50 ${card}`}><RefreshCw className={`h-5 w-5 ${loading ? "animate-spin" : ""}`} /></button></div></header>
    {message && <p role="alert" className="rounded-xl border border-red-300 bg-red-50 p-3 text-sm font-bold text-red-700">{message}</p>}
    {loading ? <div className={`grid min-h-64 place-items-center rounded-2xl border ${card}`}><Loader2 className="h-9 w-9 animate-spin text-primary" /></div> : !stats.length ? <div className={`rounded-2xl border border-dashed p-12 text-center ${card}`}><Activity className="mx-auto h-10 w-10 text-primary" /><p className="mt-3 font-black">Statistics appear after ball-by-ball scoring begins.</p></div> : <><div className="flex flex-wrap gap-2 print:hidden"><ActionButton icon={<Download className="h-4 w-4" />} label={exporting ? "Creating PNG..." : "Download PNG"} onClick={() => void downloadPng()} disabled={exporting} /><ActionButton icon={<FileDown className="h-4 w-4" />} label="Export CSV" onClick={downloadCsv} /><ActionButton icon={<Share2 className="h-4 w-4" />} label="Share" onClick={() => void shareReport()} /><ActionButton icon={<Printer className="h-4 w-4" />} label="Print / Save PDF" onClick={() => window.print()} /></div><div ref={reportRef} className={`space-y-6 rounded-2xl p-3 sm:p-5 ${admin ? "bg-background" : "bg-slate-50"}`}><MvpLeaderboard rows={mvpStats} cardClass={card} /><div className={`rounded-2xl border p-5 ${card}`}><div className="mb-4"><p className="text-xs font-black uppercase tracking-[0.18em] text-primary">CrickPulse Analytics</p><h2 className="mt-1 text-xl font-black">{tournamentName} Performance Chart</h2></div><div className="h-72 w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 8, right: 8, left: -18, bottom: 28 }}><CartesianGrid strokeDasharray="3 3" opacity={0.18} /><XAxis dataKey="name" angle={-22} textAnchor="end" interval={0} height={60} fontSize={11} /><YAxis fontSize={11} /><Tooltip /><Bar dataKey="runs" fill="#06b6d4" radius={[6, 6, 0, 0]} /><Bar dataKey="wickets" fill="#10b981" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer></div><div className="mt-3 flex justify-center gap-5 text-xs font-bold"><span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-cyan-500" />Runs</span><span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-emerald-500" />Wickets</span></div></div><div className="grid gap-6 lg:grid-cols-2"><Leaderboard title="Top Run Scorers" icon={<Trophy className="h-6 w-6 text-amber-500" />} rows={batting.slice(0, 10)} mode="batting" cardClass={card} /><Leaderboard title="Top Wicket Takers" icon={<Target className="h-6 w-6 text-emerald-500" />} rows={bowling.slice(0, 10)} mode="bowling" cardClass={card} /></div></div></>}
  </div>;
}

function MvpLeaderboard({ rows, cardClass }: { rows: MvpTournamentStat[]; cardClass: string }) {
  return <section className={`overflow-hidden rounded-2xl border shadow-sm ${cardClass}`}><header className="flex flex-col gap-2 border-b border-inherit bg-gradient-to-r from-amber-500/15 via-primary/10 to-rose-500/15 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><Medal className="h-7 w-7 text-amber-500" /><div><h2 className="text-xl font-black">Most Valuable Player (MVP) Leaderboard</h2><p className="text-xs text-muted-foreground">Automatically calculated from completed matches</p></div></div><span className="inline-flex items-center gap-1 text-xs font-black uppercase tracking-wider text-primary"><Calculator className="h-4 w-4" />Points based</span></header>{rows.length ? <div className="divide-y divide-inherit">{rows.slice(0, 10).map((row, index) => <article key={row.playerId} className="grid gap-3 p-4 sm:grid-cols-[2rem_3rem_minmax(0,1fr)_auto] sm:items-center sm:p-5"><span className="text-center text-lg font-black text-amber-500">#{index + 1}</span>{row.photoUrl ? <img src={row.photoUrl} alt={`${row.playerName} profile`} className="h-12 w-12 rounded-full border-2 border-amber-400/40 object-cover object-top" /> : <span className="grid h-12 w-12 place-items-center rounded-full bg-primary/10 text-sm font-black text-primary">{row.playerName.split(/\s+/).slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span>}<div className="min-w-0"><p className="truncate font-black">{row.playerName}</p><p className="truncate text-xs text-muted-foreground">{row.teamName} · {row.matches} completed match{row.matches === 1 ? "" : "es"}</p><div className="mt-2 flex flex-wrap gap-1.5 text-[0.65rem] font-bold"><MvpPointItem label="Runs" detail={`${row.runs} × 1`} points={row.runPoints} /><MvpPointItem label="Boundaries" detail={`${row.fours} fours + ${row.sixes} sixes`} points={row.boundaryBonus} /><MvpPointItem label="Wickets" detail={`${row.wickets} × 25`} points={row.wicketPoints} /><MvpPointItem label="Maidens" detail={`${row.maidenOvers} × 12`} points={row.maidenPoints} /><MvpPointItem label="Fielding" detail={`${row.catches} C · ${row.stumpings} St · ${row.runOuts} RO`} points={row.fieldingPoints} /><MvpPointItem label="POM" detail={`${row.playerOfMatchAwards} × 20`} points={row.playerOfMatchPoints} /></div></div><div className="text-left sm:text-right"><p className="text-3xl font-black text-primary">{row.mvpPoints}</p><p className="text-[0.65rem] font-black uppercase tracking-wider text-muted-foreground">MVP points</p></div></article>)}</div> : <p className="p-8 text-center text-sm font-bold text-muted-foreground">MVP rankings appear after the first completed match.</p>}<footer className="border-t border-inherit bg-muted/40 px-5 py-3 text-[0.68rem] font-semibold leading-5 text-muted-foreground">1 run = 1 · Four = +1 · Six = +2 · Wicket = +25 · Maiden = +12 · Catch = +8 · Stumping/direct-hit run out = +12 · Player of the Match = +20</footer></section>;
}

function MvpPointItem({ label, detail, points }: { label: string; detail: string; points: number }) {
...
function Leaderboard({ title, icon, rows, mode, cardClass }: { title: string; icon: React.ReactNode; rows: PlayerTournamentStat[]; mode: "batting" | "bowling"; cardClass: string }) {
