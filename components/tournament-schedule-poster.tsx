...
export function TournamentSchedulePoster({ tournaments, teams, matches }: { tournaments: PosterTournament[]; teams: PosterTeam[]; matches: PosterMatch[] }) {
  ...
  const pages = Math.max(1, Math.ceil(datedFixtures.length / MATCHES_PER_POSTER));
  const visibleFixtures = datedFixtures.slice(page * MATCHES_PER_POSTER, (page + 1) * MATCHES_PER_POSTER);
  ...
  const selectMatchDate = (date: string) => { setMatchDate(date); setPage(0); };
  const download = async () => {
    if (!posterRef.current || !tournament || !visibleFixtures.length || downloading) return;
    setDownloading(true);
    ...
    try {
      ...
      await downloadPosterDataUrl(dataUrl, `${slug}-${matchDate}-match-schedule${pages > 1 ? `-${page + 1}` : ""}-4k.jpg`);
    } finally {
      restoreImages?.();
  ...
  return <section className="overflow-hidden rounded-2xl border border-primary/30 bg-card shadow-lg">
    ...
    {!tournamentId ? <div className="grid min-h-52 place-items-center p-8 text-center text-muted-foreground"><div><CalendarDays className="mx-auto h-10 w-10 text-primary"/><p className="mt-3 font-bold">Select a tournament to create its schedule poster.</p></div></div> : !fixtures.length ? <div className="p-10 text-center text-sm font-bold text-muted-foreground">No matches are scheduled for this tournament yet.</div> : !availableDates.length ? <div className="p-10 text-center text-sm font-bold text-muted-foreground">These matches do not have dates yet. Add match dates before creating a day poster.</div> : !matchDate ? <div className="p-10 text-center text-sm font-bold text-muted-foreground">Select a match date to preview that day&apos;s schedule.</div> : <div className="space-y-4 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-muted-foreground">{formatDate(matchDate)} · {datedFixtures.length} matches{pages > 1 ? ` · Poster ${page + 1} of ${pages}` : ""}</p><div className="flex flex-wrap gap-2">{pages > 1 && <><button type="button" aria-label="Previous poster" onClick={() => setPage((value) => Math.max(0, value - 1))} disabled={page === 0} className="control"><ChevronLeft className="h-4 w-4"/></button><button type="button" aria-label="Next poster" onClick={() => setPage((value) => Math.min(pages - 1, value + 1))} disabled={page === pages - 1} className="control"><ChevronRight className="h-4 w-4"/></button></>}<button type="button" onClick={() => void download()} disabled={downloading} className="inline-flex min-h-10 items-center rounded-xl bg-gradient-to-r from-[#70e453] to-[#20c997] px-5 text-sm font-black text-[#041129] shadow-lg disabled:opacity-50"><Download className="mr-2 h-4 w-4"/>{downloading ? "Creating 4K poster…" : "Download This Day · 4K JPG"}</button></div></div>
      <div className="schedule-poster-preview overflow-x-auto rounded-xl bg-[radial-gradient(circle_at_center,rgba(37,99,235,.18),transparent_60%)] p-4">
        <div ref={posterRef} className="schedule-poster relative h-[1350px] w-[1080px] shrink-0 overflow-hidden bg-[#050d23] px-[58px] py-[54px] text-white" style={{ fontFamily: "Arial, Helvetica, sans-serif", transform: "scale(.5)", transformOrigin: "top left", marginRight: "-540px", marginBottom: "-675px" }}>
          ...
          <div className="relative z-10 flex h-full flex-col">
            ...
            <div className="py-7 text-center"><p className="text-[17px] font-black uppercase tracking-[.45em] text-sky-300">{formatDate(matchDate)}</p><h1 className="mt-2 text-[54px] font-black uppercase tracking-tight">Match Day Schedule</h1><div className="mx-auto mt-4 h-1.5 w-32 rounded-full bg-gradient-to-r from-[#70e453] to-cyan-400"/></div>
            <div className="flex-1 space-y-4">{visibleFixtures.map((match) => <FixtureRow key={match.id} match={match} left={team(match.team_a_id)} right={team(match.team_b_id)}/>)}</div>
            <footer className="mt-7 flex items-center justify-between border-t border-white/15 pt-5 text-[13px] font-bold uppercase tracking-[.14em] text-slate-300"><span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-[#70e453]"/>{tournament?.venue || visibleFixtures[0]?.ground || "Venue to be announced"}</span><span>Live score · Player stats · Every ball.</span></footer>
          </div>
        </div>
      </div>
    </div>}
