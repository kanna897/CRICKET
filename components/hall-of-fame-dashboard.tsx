...
function AwardCard({ title, subtitle, player, icon: Icon, accent, cardClass, finalized }: { title: string; subtitle: string; player?: PlayerTournamentStat; icon: typeof Trophy; accent: string; cardClass: string; finalized: boolean }) {
  return <article className={`relative flex min-h-64 h-full flex-col overflow-hidden rounded-2xl border p-5 shadow-sm ${cardClass}`}><div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${accent}`} /><div className="flex items-start justify-between gap-3"><span className={`grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br text-white shadow-lg ${accent}`}><Icon className="h-6 w-6" /></span><span className="rounded-full bg-primary/10 px-2.5 py-1 text-[0.65rem] font-black uppercase tracking-widest text-primary">{finalized ? "Winner" : "Current leader"}</span></div><div className="mt-6 flex items-center gap-4">{player?.photoUrl ? <Image src={player.photoUrl} alt="" width={64} height={64} sizes="64px" className="h-16 w-16 shrink-0 rounded-full object-cover object-top ring-4 ring-primary/25" /> : <span className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-primary/10 text-lg font-black text-primary">{player ? player.playerName.slice(0, 2).toUpperCase() : <UserRound className="h-7 w-7" />}</span>}<div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">{title}</p><h2 className="mt-1 truncate text-xl font-black">{player?.playerName || "Awaiting performances"}</h2><p className="truncate text-sm text-muted-foreground">{player?.teamName || "No verified leader yet"}</p></div></div><p className="mt-auto pt-5 text-sm font-bold">{subtitle || "Updates automatically after verified match records."}</p></article>;
}
