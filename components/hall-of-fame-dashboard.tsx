"use client";
/* eslint-disable @typescript-eslint/no-explicit-any -- live scoring tables are newer than generated local Supabase types */

import { useCallback, useEffect, useMemo, useState } from "react";
import { Award, Crown, Loader2, Medal, RefreshCw, ShieldCheck, Sparkles, Target, Trophy } from "lucide-react";
...
export function HallOfFameDashboard({ admin = false, organizerId, isMasterAdmin = false }: { admin?: boolean; organizerId?: string; isMasterAdmin?: boolean }) {
  ...
  useEffect(() => { void (async () => {
    let query = (supabase.from("tournaments") as any).select("id,name,status").order("created_at", { ascending: false });
  ...
  const load = useCallback(async () => {
    ...
    const [playerResult, ballResult] = await Promise.all([
      teams.length ? (supabase.from("players") as any).select("id,name,team_id,photo_url").in("team_id", teams.map((team) => team.id)) : Promise.resolve({ data: [], error: null }),
      innings.length ? (supabase.from("ball_by_ball") as any).select("innings_id,over_number,batsman_id,bowler_id,player_out_id,fielder_id,runs,extras,extras_type,is_legal,is_wicket,dismissal_type").in("innings_id", innings.map((item) => item.id)) : Promise.resolve({ data: [], error: null }),
    ]);
  ...
  return <div className="mx-auto max-w-6xl space-y-6"><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.2em] text-amber-500">CrickPulse Awards</p><h1 className={`mt-1 flex items-center gap-3 text-3xl font-black ${admin ? "text-foreground" : "text-slate-900"}`}><Medal className="h-8 w-8 text-amber-500" />Hall of Fame</h1><p className={`mt-2 ${admin ? "text-muted-foreground" : "text-slate-500"}`}>Final awards calculated from verified performances after the tournament is completed.</p></div><div className="flex gap-2"><select aria-label="Tournament" value={selectedTournament} onChange={(event) => setSelectedTournament(event.target.value)} className={`min-w-0 flex-1 rounded-xl border px-3 py-2.5 font-bold sm:min-w-64 ${card}`}>{tournaments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button type="button" onClick={() => void load()} disabled={!selectedTournament || loading} aria-label="Refresh Awards" className={`rounded-xl border px-3 text-primary disabled:opacity-50 ${card}`}><RefreshCw className={`h-5 w-5 ${loading ? "animate-spin" : ""}`} /></button></div></header>
    {message && <p role="alert" className="rounded-xl border border-red-300 bg-red-50 p-3 text-sm font-bold text-red-700">{message}</p>}
    {loading ? <div className={`grid min-h-64 place-items-center rounded-2xl border ${card}`}><Loader2 className="h-9 w-9 animate-spin text-primary" /></div> : tournamentCompleted && stats.length ? <div className="grid items-stretch gap-4 sm:grid-cols-2 lg:grid-cols-3">{awards.map((award) => <AwardCard key={award.title} {...award} cardClass={card} />)}</div> : <div className={`rounded-2xl border border-dashed p-12 text-center ${card}`}><Award className="mx-auto h-10 w-10 text-amber-500" /><p className="mt-3 font-black">{tournamentCompleted ? "Verified performance data is not available yet." : "Final awards will be announced after the tournament is completed."}</p></div>}
  </div>;
}

function AwardCard({ title, subtitle, player, icon: Icon, accent, cardClass }: { title: string; subtitle: string; player?: PlayerTournamentStat; icon: typeof Trophy; accent: string; cardClass: string }) {
  if (!player) return null;
  return <article className={`relative flex min-h-64 h-full flex-col overflow-hidden rounded-2xl border p-5 shadow-sm ${cardClass}`}><div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${accent}`} /><div className="flex items-start justify-between gap-3"><span className={`grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br text-white shadow-lg ${accent}`}><Icon className="h-6 w-6" /></span><span className="rounded-full bg-amber-500/10 px-2.5 py-1 text-[0.65rem] font-black uppercase tracking-widest text-amber-600">Award</span></div><div className="mt-6 flex items-center gap-4">{player.photoUrl ? <img src={player.photoUrl} alt="" className="h-16 w-16 shrink-0 rounded-full object-cover ring-4 ring-amber-400/25" /> : <span className="grid h-16 w-16 shrink-0 place-items-center rounded-full bg-primary/10 text-lg font-black text-primary">{player.playerName.slice(0, 2).toUpperCase()}</span>}<div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.16em] text-amber-500">{title}</p><h2 className="mt-1 truncate text-xl font-black">{player.playerName}</h2><p className="truncate text-sm text-muted-foreground">{player.teamName}</p></div></div><p className="mt-auto pt-5 text-sm font-bold">{subtitle}</p></article>;
}
