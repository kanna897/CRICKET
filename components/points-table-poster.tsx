...
export function PointsTablePoster({ tournamentName, tournamentLogo, rows, teams }: { tournamentName: string; tournamentLogo?: string | null; rows: StandingRow[]; teams: Team[] }) {
  ...
  const download = async () => {
    ...
    setDownloading(true);
    try {
      const dataUrl = await toJpeg(ref.current, { cacheBust: true, pixelRatio: 1.75, quality: 0.96, backgroundColor: "#050b26", skipFonts: true });
      const link = document.createElement("a");
      link.download = `${tournamentName.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-points-table.jpg`;
      link.href = dataUrl;
      link.click();
  ...
  return <section className="space-y-3">
    <div className="flex justify-end"><button type="button" onClick={() => void download()} disabled={downloading} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-300 px-4 py-2.5 text-sm font-black text-slate-950 shadow-lg transition hover:-translate-y-0.5 disabled:opacity-50"><Download className="h-4 w-4" />{downloading ? "Creating JPG..." : "Download Points Table JPG"}</button></div>
