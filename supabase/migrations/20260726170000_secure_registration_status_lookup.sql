create table if not exists private.registration_lookup_limits (
...
alter table public.player_registrations
  add column if not exists contact_number_normalized text
  generated always as (regexp_replace(contact_number, '[^0-9]+', '', 'g')) stored;

create index if not exists idx_player_registrations_secure_lookup
  on public.player_registrations (tracking_code, contact_number_normalized);

alter table private.registration_lookup_limits enable row level security;

revoke all on table private.registration_lookup_limits from public, anon, authenticated;
...
revoke all on function public.lookup_player_registration(text, text)
  from public, anon, authenticated;
drop function if exists public.lookup_player_registration(text, text);
