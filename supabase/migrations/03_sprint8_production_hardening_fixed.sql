-- Sprint 8 Production Hardening (Fixed)
...
-- Use partial unique indexes to properly handle NULL tournament_id for super_admins
CREATE UNIQUE INDEX IF NOT EXISTS user_roles_super_admin_idx ON public.user_roles (user_id, role) WHERE tournament_id IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS user_roles_tournament_idx ON public.user_roles (user_id, tournament_id, role) WHERE tournament_id IS NOT NULL;
...
-- Drop new policies if this script is re-run
...
DROP POLICY IF EXISTS "Admin/Scorer Write Matches" ON public.matches;
DROP POLICY IF EXISTS "Admin/Scorer Write Innings" ON public.innings;
DROP POLICY IF EXISTS "Admin/Scorer Write Ball By Ball" ON public.ball_by_ball;
...
-- Use DROP TRIGGER IF EXISTS followed by CREATE TRIGGER for PG compatibility
DROP TRIGGER IF EXISTS update_tournaments_modtime ON public.tournaments;
CREATE TRIGGER update_tournaments_modtime BEFORE UPDATE ON public.tournaments FOR EACH ROW EXECUTE FUNCTION update_modified_column();
...
DROP TRIGGER IF EXISTS update_players_modtime ON public.players;
CREATE TRIGGER update_players_modtime BEFORE UPDATE ON public.players FOR EACH ROW EXECUTE FUNCTION update_modified_column();
...
DROP TRIGGER IF EXISTS update_player_statistics_modtime ON public.player_statistics;
CREATE TRIGGER update_player_statistics_modtime BEFORE UPDATE ON public.player_statistics FOR EACH ROW EXECUTE FUNCTION update_modified_column();

DROP TRIGGER IF EXISTS update_points_table_modtime ON public.points_table;
CREATE TRIGGER update_points_table_modtime BEFORE UPDATE ON public.points_table FOR EACH ROW EXECUTE FUNCTION update_modified_column();
...
DROP POLICY IF EXISTS "Admin Write Player Statistics" ON public.player_statistics;
...
DROP POLICY IF EXISTS "Admin Write Points Table" ON public.points_table;
