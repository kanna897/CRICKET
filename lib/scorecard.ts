...
export function buildScorecard(innings: ScorecardInnings, balls: ScorecardBall[], players: ScorecardPlayer[]): InningsScorecard {
  ...
  return { batting, bowling, extras: balls.reduce((sum, ball) => sum + ball.extras, 0), total: innings.total_runs, wickets: innings.total_wickets, overs: overs(innings.balls_bowled), fallOfWickets };
}
