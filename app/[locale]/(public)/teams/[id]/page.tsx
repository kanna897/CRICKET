...
export default function PublicTeamPage() {
  ...
  return <><PublicNav /><main className="mx-auto max-w-4xl p-4 sm:p-7">
    <Link href="/teams" className="mb-5 inline-flex items-center gap-1 text-sm font-bold text-sky-700"><ArrowLeft className="h-4 w-4" />All teams</Link>
    <header className="mb-6 flex items-center gap-4 rounded-2xl bg-gradient-to-r from-sky-50 to-emerald-50 p-5">{team?.logo_url ? <img src={team.logo_url} alt="" className="h-16 w-16 rounded-full bg-white object-cover" /> : null}<div><p className="text-xs font-black uppercase tracking-widest text-sky-600">Team squad</p><h1 className="text-3xl font-black text-slate-900">{team?.name || "Team"}</h1></div></header>
