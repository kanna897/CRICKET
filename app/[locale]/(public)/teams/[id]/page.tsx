...
function PlayerAvatar({ player, large = false }: { player: Player; large?: boolean }) {
  ...
  return player.photo_url ? <Image unoptimized width={128} height={128} src={player.photo_url} alt="" className={`${size} shrink-0 rounded-full border border-sky-200 object-cover`} /> : <span className={`${size} flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky-100 to-emerald-100 font-black text-sky-700`}>{player.name.slice(0, 1)}</span>;
}
