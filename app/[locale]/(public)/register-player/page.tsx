...
export default function PublicPlayerRegistrationPage() {
  ...
  if (submitted) return <main className="mx-auto grid min-h-[70vh] max-w-2xl place-items-center p-5"><section className="w-full rounded-3xl border border-emerald-300 bg-emerald-50 p-8 text-center text-emerald-950 shadow-xl dark:border-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-50"><CheckCircle2 className="mx-auto h-14 w-14 text-emerald-600"/><h1 className="mt-4 text-3xl font-black">Registration submitted</h1><p className="mt-2">Organizer approve பண்ணிய பிறகு player directory-ல் add ஆகும்.</p><div className="mx-auto mt-6 max-w-sm rounded-2xl border border-emerald-300 bg-white/70 p-4 dark:bg-black/20"><p className="text-xs font-black uppercase tracking-widest">Your tracking code</p><p className="mt-2 font-mono text-3xl font-black tracking-wider">{trackingCode}</p><button type="button" onClick={()=>void navigator.clipboard.writeText(trackingCode)} className="mt-3 inline-flex items-center gap-2 rounded-lg border border-emerald-500 px-3 py-2 text-sm font-bold"><Clipboard className="h-4 w-4"/>Copy code</button></div><p className="mt-4 text-sm">இந்த code + contact number save பண்ணுங்கள்; status பார்க்க இது தேவை.</p></section></main>;
