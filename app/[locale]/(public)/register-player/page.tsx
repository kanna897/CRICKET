...
export default function PublicPlayerRegistrationPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setMessage(duplicate ? "This contact number already has a pending or approved registration." : (failure?.message || "Registration could not be submitted."));
    } finally { setSaving(false); }
  }

  if (submitted) return <main className="mx-auto grid min-h-[70vh] max-w-3xl place-items-center p-5"><section className="w-full rounded-3xl border border-emerald-300 bg-emerald-50 p-8 text-center text-emerald-950 shadow-xl dark:border-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-50"><CheckCircle2 className="mx-auto h-14 w-14 text-emerald-600"/><h1 className="mt-4 text-3xl font-black">Registration submitted</h1><p className="mt-2">Your player profile will appear after organizer approval.</p><div className="mx-auto mt-6 max-w-sm"><div className="rounded-2xl border border-emerald-300 bg-white/70 p-4 dark:bg-black/20"><p className="text-xs font-black uppercase tracking-widest">Registration number</p><p className="mt-2 font-mono text-4xl font-black">{registrationNumber === null ? "Generating…" : String(registrationNumber).padStart(2, "0")}</p><p className="mt-5 text-xs font-black uppercase tracking-widest">Tracking code</p><p className="mt-2 font-mono text-2xl font-black tracking-wider">{trackingCode}</p><button type="button" onClick={()=>void navigator.clipboard.writeText(trackingCode)} className="mt-3 inline-flex items-center gap-2 rounded-lg border border-emerald-500 px-3 py-2 text-sm font-bold"><Clipboard className="h-4 w-4"/>Copy code</button></div>{playerCardUrl ? <a href={playerCardUrl} download className="group overflow-hidden rounded-2xl border border-emerald-300 bg-white p-2 text-left"><Image unoptimized width={128} height={128} src={playerCardUrl} alt="Generated player profile card" className="aspect-square w-full rounded-xl object-cover"/><span className="mt-2 block text-center text-sm font-black text-emerald-800">Download Player Card JPG</span></a> : <div className="hidden">Player card will be generated when an active tournament template is selected.</div>}</div><p className="mt-4 text-sm">Save this tracking code and contact number to check your registration status later.</p></section></main>;
