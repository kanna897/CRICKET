...
export function PublicLiveMatchClient() {
  ...
  return <main className="mx-auto max-w-3xl space-y-4 p-3 sm:p-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><Link href="/fixtures" className="inline-flex items-center gap-2 px-1 text-sm font-bold text-sky-700 hover:text-sky-900 dark:text-sky-300"><ArrowLeft className="h-4 w-4" />Back to matches</Link><button type="button" onClick={toggleAlerts} className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-black shadow-sm ${alertsEnabled ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-slate-300 bg-white text-slate-800"}`}>{alertsEnabled?<Bell className="h-4 w-4"/>:<BellOff className="h-4 w-4"/>}{alertsEnabled?"Live alerts on":"Enable live alerts"}</button></div>
    <nav aria-label="Match sections" className="flex items-center justify-between gap-1 rounded-2xl border border-sky-400/25 bg-[#071427]/90 p-1.5 shadow-lg backdrop-blur-md">
      <Link href={`/match/${id}`} className="flex-1 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-2 py-2 text-center text-xs font-black text-white shadow-sm">
        🔴 Live
      </Link>
      <Link href={`/match/${id}/scorecard`} className="flex-1 rounded-xl px-2 py-2 text-center text-xs font-bold text-slate-300 transition hover:bg-white/10 hover:text-white">
        📄 Scorecard
      </Link>
      <Link href={`/match/${id}/teamsheet`} className="flex-1 rounded-xl px-2 py-2 text-center text-xs font-bold text-slate-300 transition hover:bg-white/10 hover:text-white">
        👥 Playing XI
      </Link>
      <Link href={`/match/${id}/analytics`} className="flex-1 rounded-xl px-2 py-2 text-center text-xs font-bold text-slate-300 transition hover:bg-white/10 hover:text-white">
        📊 Analytics
      </Link>
    </nav>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Current over</p><p className="mt-1 font-black text-slate-900">Over {currentOver || Math.floor((innings?.balls_bowled || 0) / 6) + 1}</p></div><p className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">Recent delivery</p></div><div className="mt-4 flex min-h-12 items-center gap-2 overflow-x-auto pb-1"><div className="flex flex-nowrap items-center gap-2">{currentOverBalls.length ? currentOverBalls.map((ball) => <span key={ball.id} title={`${ball.over_number}.${ball.ball_number}`} className={`grid h-11 w-11 shrink-0 place-items-center rounded-full border text-sm font-black ${ball.is_wicket ? "border-red-500 bg-red-500 text-white" : ball.extras_type ? "border-amber-400 bg-amber-50 text-amber-800" : ball.runs === 4 || ball.runs === 6 ? "border-emerald-400 bg-emerald-50 text-emerald-800" : "border-slate-300 bg-slate-50 text-slate-800"}`}>{ballLabel(ball)}</span>) : <p className="text-sm text-slate-500">Waiting for the first delivery.</p>}</div></div></section>
