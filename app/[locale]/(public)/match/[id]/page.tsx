...
export default function PublicLiveMatch() {
  ...
  return <main className="mx-auto max-w-3xl space-y-4 p-3 sm:p-6">
    ...
    <section className="rounded-2xl border border-cyan-200/70 bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-4"><div><p className="truncate text-sm font-bold text-slate-800">{teamName(match.team_a_id)}</p><p className="mt-1 text-2xl font-black text-cyan-700">{teamAWinChance}%</p></div><div className="text-right"><p className="truncate text-sm font-bold text-slate-800">{teamName(match.team_b_id)}</p><p className="mt-1 text-2xl font-black text-emerald-700">{teamBWinChance}%</p></div></div>
