...
export default function PublicPlayerProfilePage() {
  ...
  if (loading) return <><PublicNav /><main className="grid min-h-[60vh] place-items-center text-foreground">{t("loading")}</main></>;
  if (!player) return <><PublicNav /><main className="p-10 text-center text-foreground">{t("notFound")}</main></>;
  const symbol = roleSymbol(player.playing_role);
  return <><PublicNav /><main className="mx-auto max-w-5xl space-y-5 p-4 sm:p-7"><Link href={team ? `/teams/${team.id}` : "/teams"} className="inline-flex items-center gap-2 font-bold text-primary"><ArrowLeft className="h-4 w-4" />{t("back")}</Link>
    <section className="overflow-hidden rounded-3xl border border-cyan-300/30 bg-gradient-to-br from-[#07162f] via-[#0b2d59] to-[#073c3a] p-5 text-white shadow-2xl sm:p-8"><div className="flex flex-col items-center gap-5 sm:flex-row">{player.photo_url ? <Image unoptimized width={128} height={128} src={player.photo_url} alt={player.name} className="h-36 w-36 rounded-3xl border-4 border-white object-cover object-top shadow-xl" /> : <span className="grid h-36 w-36 place-items-center rounded-3xl border-4 border-white/70 bg-white/10"><User className="h-16 w-16" /></span>}<div className="min-w-0 text-center sm:text-left"><span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs font-black uppercase tracking-wider">{symbol} {player.playing_role || "Player"}</span><h1 className="mt-3 text-3xl font-black sm:text-5xl">{player.name}</h1><p className="mt-2 flex items-center justify-center gap-2 text-cyan-100 sm:justify-start"><Shield className="h-4 w-4" />{team?.name || "Unassigned team"}</p><p className="mt-2 text-sm text-white/65">{player.batting_style || "Batting style not specified"} · {player.bowling_style || "Bowling style not specified"}</p></div></div></section>
    <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6"><Metric icon={<Award />} label={t("matches")} value={matchCount} /><Metric icon={<Target />} label={t("runs")} value={stats.runs} /><Metric icon={<CircleDot />} label={t("wickets")} value={stats.wickets} /><Metric icon={<Target />} label={t("highScore")} value={stats.high} /><Metric icon={<Award />} label={t("average")} value={stats.average.toFixed(1)} /><Metric icon={<Shield />} label={t("fielding")} value={stats.catches + stats.stumpings + stats.runOuts} /></section>
    <section className="rounded-2xl border border-border bg-card p-5 text-foreground"><h2 className="text-xl font-black">{t("recent")}</h2><p className="mt-1 text-sm text-muted-foreground">{t("latest")}</p><div className="mt-4 flex flex-wrap gap-3">{stats.recent.length ? stats.recent.map((score, index) => <span key={`${score}-${index}`} className="grid h-12 w-12 place-items-center rounded-full border border-primary/30 bg-primary/10 text-lg font-black text-primary">{score}</span>) : <p className="text-sm text-muted-foreground">{t("noInnings")}</p>}</div></section>
