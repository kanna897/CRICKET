...
type MatchEditForm = { team_a_id: string; team_b_id: string; match_date: string; match_time: string; ground: string; overs_per_match: string };
type GeneratorForm = { tournamentId: string; startDate: string; matchTime: string; ground: string; restDays: string; matchesPerDay: string; matchDuration: string; breakMinutes: string; dayMode: MatchDayMode; weekdays: number[]; customDates: string };
...
export default function MatchesPage() {
  ...
  const [generator, setGenerator] = useState<GeneratorForm>({ tournamentId: "", startDate: "", matchTime: "09:00", ground: "", restDays: "1", matchesPerDay: "4", matchDuration: "60", breakMinutes: "15", dayMode: "weekdays", weekdays: [0, 6], customDates: "" });
  ...
  useEffect(() => {
    ...
    let tournamentQuery = supabase.from("tournaments").select("*").is("deleted_at", null).order("created_at", { ascending: false });
      ...
      const datedMatches = manageableMatches
        .filter((match) => match.match_date)
        .sort((left, right) => `${left.match_date} ${left.match_time || ""}`.localeCompare(`${right.match_date} ${right.match_time || ""}`));
      const today = new Date().toLocaleDateString("en-CA");
      setSelectedMatchDate((current) => current || datedMatches.find((match) => match.match_date! >= today)?.match_date || datedMatches[0]?.match_date || "");
  ...
  const openMatchEditor = (match: Match) => {
    ...
    setEditForm({ team_a_id: match.team_a_id, team_b_id: match.team_b_id, match_date: match.match_date || "", match_time: match.match_time || "", ground: match.ground || "", overs_per_match: String(match.overs_per_match) });
  };
  const saveMatchEdit = async () => {
    if (!editingMatch || editingMatch.status !== "scheduled") return setMessage("Only scheduled matches can be edited.");
    const overs = Number(editForm.overs_per_match);
    if (!editForm.team_a_id || !editForm.team_b_id) return setMessage("Select both teams.");
    if (editForm.team_a_id === editForm.team_b_id) return setMessage("Choose two different teams.");
    if (!editForm.match_date || !editForm.match_time) return setMessage("Match date and time are required.");
    if (!Number.isInteger(overs) || overs < 1 || overs > 100) return setMessage("Overs must be a whole number between 1 and 100.");
    setSavingEdit(true); setMessage("");
    try {
      const changes = { team_a_id: editForm.team_a_id, team_b_id: editForm.team_b_id, match_date: editForm.match_date, match_time: editForm.match_time, ground: editForm.ground.trim() || null, overs_per_match: overs };
      const { data, error } = await supabase.from("matches").update(changes).eq("id", editingMatch.id).eq("status", "scheduled").select("*").maybeSingle();
      if (error) throw error;
      if (!data) throw new Error("This match is no longer scheduled. Refresh and try again.");
      setMatches((current) => current.map((match) => match.id === data.id ? data as Match : match));
      setSelectedMatchDate(editForm.match_date);
      setEditingMatch(null);
      setMessage(`Match #${editingMatch.match_number || "—"} schedule updated successfully.`);
    } catch (reason) {
      setMessage(reason instanceof Error ? reason.message : "Match schedule could not be updated.");
    } finally { setSavingEdit(false); }
  };
  const matchDates = [...new Set(matches.map((match) => match.match_date).filter((date): date is string => Boolean(date)))].sort();
  const selectedDateIndex = matchDates.indexOf(selectedMatchDate);
  const visibleMatches = matches
    .filter((match) => match.match_date === selectedMatchDate)
    .sort((left, right) => `${left.match_time || "99:99"} ${left.match_number || 999999}`.localeCompare(`${right.match_time || "99:99"} ${right.match_number || 999999}`));
  const editingTeamOptions = editingMatch?.tournament_id
    ? teams.filter((item) => item.tournament_id === editingMatch.tournament_id || item.id === editingMatch.team_a_id || item.id === editingMatch.team_b_id)
    : teams;
  const generatePreview = () => {
    setMessage("");
    setFixturePreview([]);
    const tournamentTeams = teams.filter((team) => team.tournament_id === generator.tournamentId);
    if (!generator.tournamentId || (generator.dayMode !== "custom" && !generator.startDate)) return setMessage("Tournament and first match date are required.");
    ...
    try {
      ...
      const dayCount = Math.max(...scheduled.map((pair) => pair.dayIndex), -1) + 1;
      const customDates = generator.customDates.split(/[\s,]+/).map((date) => date.trim()).filter(Boolean);
      const firstDate = generator.dayMode === "custom" ? customDates.sort()[0] || "" : generator.startDate;
      const matchDayDates = generateMatchDayDates({ mode: generator.dayMode, startDate: firstDate, dayCount, intervalDays: Number(generator.restDays) || 1, weekdays: generator.weekdays, customDates });
      ...
      const previewMatches = scheduled.flatMap((pair) => {
          const date = matchDayDates[pair.dayIndex];
  ...
  return (
    <div className="admin-themed-page space-y-6">
      ...
      {generatorOpen&&<ScheduleModeFields generator={generator} onChange={(changes)=>{setGenerator({...generator,...changes});setFixturePreview([]);}}/>}
      {editingMatch&&<div role="dialog" aria-modal="true" aria-labelledby="match-edit-title" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/70 p-4"><section className="w-full max-w-2xl rounded-2xl border border-border bg-card p-5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-widest text-primary">Scheduled match</p><h2 id="match-edit-title" className="mt-1 text-xl font-black">Edit Match #{editingMatch.match_number || "—"}</h2><p className="mt-1 text-sm text-muted-foreground">Change teams when a scheduled fixture is rearranged.</p></div><button type="button" aria-label="Close match editor" onClick={()=>setEditingMatch(null)} disabled={savingEdit} className="grid h-9 w-9 place-items-center rounded-full border border-border"><X className="h-4 w-4"/></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-bold">Team A<select className="input" value={editForm.team_a_id} onChange={(event)=>setEditForm({...editForm,team_a_id:event.target.value})}><option value="">Select team</option>{editingTeamOptions.filter((item)=>item.id!==editForm.team_b_id).map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="space-y-2 text-sm font-bold">Team B<select className="input" value={editForm.team_b_id} onChange={(event)=>setEditForm({...editForm,team_b_id:event.target.value})}><option value="">Select team</option>{editingTeamOptions.filter((item)=>item.id!==editForm.team_a_id).map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="space-y-2 text-sm font-bold">Match date<input type="date" className="input" value={editForm.match_date} onChange={(event)=>setEditForm({...editForm,match_date:event.target.value})}/></label><label className="space-y-2 text-sm font-bold">Match time<input type="time" className="input" value={editForm.match_time} onChange={(event)=>setEditForm({...editForm,match_time:event.target.value})}/></label><label className="space-y-2 text-sm font-bold">Ground / Venue<input className="input" value={editForm.ground} onChange={(event)=>setEditForm({...editForm,ground:event.target.value})}/></label><label className="space-y-2 text-sm font-bold">Overs per match<input type="number" min="1" max="100" className="input" value={editForm.overs_per_match} onChange={(event)=>setEditForm({...editForm,overs_per_match:event.target.value})}/></label></div><div className="mt-5 flex justify-end gap-2"><button type="button" disabled={savingEdit} onClick={()=>setEditingMatch(null)} className="min-h-11 rounded-xl border border-border px-4 font-bold">Cancel</button><button type="button" disabled={savingEdit} onClick={()=>void saveMatchEdit()} className="inline-flex min-h-11 items-center rounded-xl bg-primary px-5 font-black text-primary-foreground disabled:opacity-50">{savingEdit?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<ShieldCheck className="mr-2 h-4 w-4"/>}Save Changes</button></div></section></div>}
      ...
      {!loading&&<TournamentSchedulePoster tournaments={tournaments} teams={teams} matches={matches}/>}
      {!loading&&matchDates.length>0&&<section className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-primary/10 text-primary"><CalendarDays className="h-5 w-5"/></span><div><p className="text-xs font-black uppercase tracking-wider text-muted-foreground">Showing match day</p><p className="font-black">{formatMatchDay(selectedMatchDate)}</p></div></div><div className="flex items-center gap-2"><button type="button" aria-label="Previous match day" disabled={selectedDateIndex<=0} onClick={()=>setSelectedMatchDate(matchDates[selectedDateIndex-1])} className="inline-flex min-h-10 items-center rounded-lg border border-border px-3 text-sm font-black disabled:opacity-40"><ChevronLeft className="mr-1 h-4 w-4"/>Previous</button><select aria-label="Select match date" className="input min-w-40" value={selectedMatchDate} onChange={(event)=>setSelectedMatchDate(event.target.value)}>{matchDates.map((date)=><option key={date} value={date}>{formatMatchDay(date)}</option>)}</select><button type="button" aria-label="Next match day" disabled={selectedDateIndex<0||selectedDateIndex>=matchDates.length-1} onClick={()=>setSelectedMatchDate(matchDates[selectedDateIndex+1])} className="inline-flex min-h-10 items-center rounded-lg border border-border px-3 text-sm font-black disabled:opacity-40">Next<ChevronRight className="ml-1 h-4 w-4"/></button></div></section>}
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        ...
        ) : visibleMatches.length === 0 ? <div className="py-16 text-center"><CalendarDays className="mx-auto mb-3 h-10 w-10 text-muted-foreground"/><h2 className="font-semibold">No matches on this date</h2><p className="mt-1 text-sm text-muted-foreground">Choose another match day using Previous, Next, or the date selector.</p></div> : <div className="divide-y divide-border">
          {visibleMatches.map((match) => <div key={match.id} className="flex flex-col gap-4 bg-[linear-gradient(135deg,rgba(11,41,93,.28),transparent_58%)] p-5 lg:flex-row lg:items-center lg:justify-between">
...
function ScheduleModeFields({ generator, onChange }: { generator: GeneratorForm; onChange: (changes: Partial<GeneratorForm>) => void }) {
  const modes: Array<[MatchDayMode, string, string]> = [
    ["continuous", "Continuous / fixed gap", "Schedule every day or use a fixed number of days between match days."],
    ["weekdays", "Selected weekdays", "Mix weekdays and weekends; all unselected calendar days are skipped."],
    ["custom", "Custom dates", "Use exact tournament dates chosen by the organizer."],
  ];
  const weekdays: Array<[string, number]> = [["Sun", 0], ["Mon", 1], ["Tue", 2], ["Wed", 3], ["Thu", 4], ["Fri", 5], ["Sat", 6]];
  return <section className="rounded-2xl border border-primary/30 bg-card p-5 shadow-lg"><div><p className="text-xs font-black uppercase tracking-widest text-primary">Match day calendar</p><h2 className="mt-1 text-xl font-black">Choose when tournament days can run</h2><p className="mt-1 text-sm text-muted-foreground">Weekend-only, mixed weekday/weekend and exact-date tournaments are supported.</p></div><div className="mt-4 grid gap-3 md:grid-cols-3">{modes.map(([value, label, help]) => <label key={value} className={`cursor-pointer rounded-xl border p-4 ${generator.dayMode === value ? "border-primary bg-primary/10" : "border-border"}`}><input type="radio" name="dayMode" className="mr-2 accent-primary" checked={generator.dayMode === value} onChange={() => onChange({ dayMode: value })}/><span className="font-black">{label}</span><span className="mt-1 block text-xs text-muted-foreground">{help}</span></label>)}</div>{generator.dayMode === "weekdays" && <div className="mt-4 rounded-xl border border-border bg-background/50 p-4"><p className="text-sm font-black">Allowed match weekdays</p><div className="mt-3 flex flex-wrap gap-2">{weekdays.map(([label, day]) => { const selected = generator.weekdays.includes(day); return <label key={day} className={`cursor-pointer rounded-lg border px-3 py-2 text-xs font-black ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}><input type="checkbox" className="sr-only" checked={selected} onChange={() => onChange({ weekdays: selected ? generator.weekdays.filter((item) => item !== day) : [...generator.weekdays, day] })}/>{label}</label>; })}</div><p className="mt-2 text-xs text-muted-foreground">Saturday and Sunday are selected by default. Add any weekdays the tournament will also use.</p></div>}{generator.dayMode === "custom" && <label className="mt-4 block space-y-2 text-sm font-bold">Exact match dates<textarea className="input min-h-24" placeholder="2026-08-22, 2026-08-23, 2026-08-29, 2026-08-30" value={generator.customDates} onChange={(event) => onChange({ customDates: event.target.value })}/><span className="block text-xs font-normal text-muted-foreground">Separate dates with commas or spaces. They are sorted and duplicate dates are removed.</span></label>}</section>;
}
