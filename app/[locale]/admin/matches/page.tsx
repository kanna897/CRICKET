...
import { CalendarPlus, ClipboardList, Loader2, LockKeyhole, Pencil, PlayCircle, ShieldCheck, Sparkles, Trash2, UnlockKeyhole, X } from "lucide-react";
...
type MatchEditForm = { match_date: string; match_time: string; ground: string; overs_per_match: string };
...
export default function MatchesPage() {
  ...
  const [editForm, setEditForm] = useState<MatchEditForm>({ match_date: "", match_time: "", ground: "", overs_per_match: "20" });
  ...
  const openMatchEditor = (match: Match) => {
    ...
    setEditForm({ match_date: match.match_date || "", match_time: match.match_time || "", ground: match.ground || "", overs_per_match: String(match.overs_per_match) });
  };
  const saveMatchEdit = async () => {
    ...
    try {
      const changes = { match_date: editForm.match_date, match_time: editForm.match_time, ground: editForm.ground.trim() || null, overs_per_match: overs };
