...
import { ArrowLeft, ArrowRightLeft, BarChart3, CheckCircle2, FileText, Loader2, Settings2, Share2, UserPlus } from "lucide-react";
...
type Ball = { id: string; over_number: number; ball_number: number; runs: number; extras: number; extras_type: string | null; is_legal: boolean; is_wicket: boolean; dismissal_type: string | null; batsman_id: string | null; non_striker_id: string | null; bowler_id: string | null; player_out_id: string | null; commentary?: string | null };
...
export default function LiveScorer() {
  ...
  useEffect(() => {
    const syncPending = async () => {
      ...
      const items = await getOfflineQueue() as OfflineQueueItem<{ ball: Record<string, unknown>; inningsId: string; next: Record<string, unknown> }>[];
      const current = items.filter((item) => item.matchId === id);
      setOfflinePending(current.length);
      let synced = 0;
      for (const item of current) {
        const ballResult = await (supabase.from("ball_by_ball") as any).insert(item.payload.ball);
        if (ballResult.error) break;
        const inningsResult = await (supabase.from("innings") as any).update(item.payload.next).eq("id", item.payload.inningsId);
        if (inningsResult.error) break;
        await removeOfflineQueueItem(item.id); synced++;
      }
  ...
  const record = async ({ runs = 0, extras = 0, extrasType, legal = true, wicket = false, dismissalType, outId, fielderId }: { runs?: number; extras?: number; extrasType?: string; legal?: boolean; wicket?: boolean; dismissalType?: string; outId?: string; fielderId?: string }) => {
    ...
    try {
      ...
      if (ballError) {
        ...
        setInnings({ ...innings, ...next }); setBalls([...balls, queuedBall]); setOfflinePending((count) => count + 1);
  ...
  const saveNewBatter = async () => {
    if (!innings || !newBatter || !eligibleNewBatters.some((player) => player.id === newBatter)) return alert("Select an available batter.");
    setSaving(true);
    try {
      await saveInnings({ [batterSlot]: newBatter });
      setNewBatterOpen(false);
    } catch { alert("Unable to update batter."); }
    finally { setSaving(false); }
  };
  ...
  const handoverScoring = async () => {
    ...
    const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([snapshot], { type: "application/json" })); link.download = `crickpulse-handover-${id}.json`; link.click(); URL.revokeObjectURL(link.href);
  ...
  return <div className="live-score-shell max-w-5xl mx-auto space-y-4 pb-12">
    ...
    <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={() => void handoverScoring()} className="control"><Share2 className="mr-1 h-4 w-4" />Scorer Handover</button><Link href={`/admin/matches/analytics/${id}`} className="control"><BarChart3 className="mr-1 h-4 w-4" />Advanced Analytics</Link><Link href={`/admin/matches/scorecard/${id}`} className="control"><FileText className="w-4 h-4 mr-1" />Scorecard & Match Summary</Link></div>
