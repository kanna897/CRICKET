...
export default function LiveScorer() {
    ...
    const [targetMethod, setTargetMethod] = useState<"manual" | "dls">("manual");
    ...
    const openAdvancedDelivery = () => {
        setAdvancedRuns(0);
        setAdvancedExtras(1);
        setAdvancedExtrasType(match?.allow_wides ? "wide" : match?.allow_no_balls ? "no_ball" : "bye");
    ...
    <section className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-sky-300 bg-sky-50 p-3 text-sky-950"><div><p className="text-sm font-black">{match.revised_overs ? `${match.target_method?.toUpperCase() || "REVISED"} adjustment active: ${effectiveOvers} overs` : "Rain / interruption controls"}</p><p className="text-xs">{match.interruption_notes || "Apply a scorer-approved revised innings length and chase target."}</p></div><button type="button" onClick={openInterruption} disabled={completed || saving || !innings} className="control border-sky-400 bg-white text-sky-950"><SlidersHorizontal className="mr-1.5 h-4 w-4"/>Adjust match</button></section>
...
function HatTrickPop({ bowlerName }: {
    bowlerName: string;
}) {
