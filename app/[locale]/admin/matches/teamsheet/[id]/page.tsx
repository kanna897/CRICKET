...
function PlayerAvatar({ player, size = "sm" }: { player: Player; size?: "sm" | "md" }) {
  ...
  return player.photo_url ? (
    <Image unoptimized width={128} height={128} src={player.photo_url} alt="" className={`${dimensions} shrink-0 rounded-full border border-sky-200 object-cover`} />
  ) : (
    ...
    <div className="admin-themed-page dashboard-page matches-page max-w-6xl mx-auto pb-12 print:max-w-full print:p-0">
      ...
      {!showOfficialSheet ? (
        ...
        <div className="rounded-xl border border-slate-300 bg-white p-5 text-slate-950 shadow-lg sm:p-8 print:m-0 print:rounded-none print:border-none print:p-0 print:shadow-none">
          ...
          <div className="mt-12 flex items-center justify-center gap-2 text-xs text-gray-400 print:mt-8"><Image unoptimized width={128} height={128} src="/brand/crickpulse-logo.png" alt="Crickpulse" className="h-5 w-20 object-contain" /> <span>Official Export • Generated {new Date().toLocaleString()}</span></div>
        </div>
      )}
    </div>
  );
